// WallpaperARView.swift — v13g (v13f + wallpaper alignment fix: refresh material on anchor correction)
//
// v13g fixes the wallpaper-misaligned-on-wall bug seen in v13e/v13f.
//
// ROOT CAUSE of v13e/v13f:
// After applying wallpaper, ARKit sometimes delivers anchor corrections
// (transform updates) that move the wall's position. The wallpaper material
// was NOT regenerated after these corrections, causing the texture and mask
// to stay in the old coordinate system, appearing offset.
//
// FIXES IN v13g:
// 1. In `session(_:didUpdate:)` – after updating the wall's transform, we
// now call `refreshWall()` to rebuild the material with the new transform.
// 2. In `placeWallpaper` – after applying wallpaper, we schedule a delayed
// refresh (0.5 sec) to catch any anchor corrections that arrive right after.
//
// All other logic (scan preview, lasso, occluder, edit mode) unchanged.
...
var lines: [String] = []
lines.append("=== OBOIA DIAGNOSTIC REPORT (v13g) ===")
...
currentMode = .idle
emit("boot", data: ["status": "AR ready (v13g)"])
...
diag(">>> startScan v13g <<<")
emit("boot", data: ["status": ">>> startScan v13g <<<"])
...
// ★ v13g FIX: schedule a second refresh after a short delay.
// This catches any anchor corrections that arrive right after wallpaper placement.
...
// ★ v13g: Trust ARKit's anchor updates and refresh material when transform changes.
...
// Reasonable clamp: accept corrections up to 15cm.
...
// ★ v13g FIX: Rebuild material after transform update so wallpaper aligns correctly.
// This ensures the mask and texture use the corrected wall coordinates.
